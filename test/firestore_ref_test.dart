import 'package:flutter_test/flutter_test.dart';
import 'package:menstrul_track/services/firestore_ref.dart';

/// LunarFlow MUST NOT use the `(default)` Firestore database: `teddy-2-20649`
/// is shared with unrelated apps, and `(default)` has ONE project-wide
/// ruleset. A named database carries its own rules. This test pins the id so a
/// rename can't silently move menstrual data under someone else's rules.
void main() {
  test('the LunarFlow database id is the named database, not (default)', () {
    expect(kLunaDatabaseId, 'lunatrack-db');
    expect(kLunaDatabaseId, isNot('(default)'));
  });
}
